...
coherent narrative, at the target length given in the user message, that:
...
def build_story_user_prompt(
    references: list[dict], premise: str, language: str, word_count: int = 350
) -> str:
    return (
        ...
        f"Target length: approximately {word_count} words (roughly "
        f"{max(1, int(word_count * 0.85))}-{int(word_count * 1.15)} words).\n"
...
def generate_story(
    ...
    user_prompt = build_story_user_prompt(references, premise, language, word_count)
    return client.chat(
        model=model,
...
The "brief" field is NOT a summary. Do not compress the beat down to its
plot function ("she enters the room and finds the letter"). Instead, WRITE
THE SCENE: a short, fully realized passage of narrative prose, dense with
concrete visual detail, that a video generator could storyboard shot-for-
shot without inventing anything on its own. For every sequence, the brief
must cover, in flowing prose (not a list):
  - the subject(s) in frame: exact pose, gesture, facial expression, gaze
    direction, and what their body/clothing/texture looks like in this
    instant,
  - the setting: architecture or terrain, key foreground/background objects,
    depth and spatial layout,
  - light and color: light source and direction, quality (hard/soft,
    warm/cool), how it falls on surfaces and skin, dominant and accent
    colors, shadow behavior,
  - atmosphere and texture: weather, particles in the air (dust, smoke,
    mist, rain), material textures (rough stone, wet fabric, rusted metal,
    glossy skin...), anything that grounds the image physically,
  - motion: what is moving within the shot (hair, fabric, fire, water,
    creatures, crowd) independent of the camera itself,
  - mood conveyed purely through what is visible, never through abstract
    labels like "tense" or "sad" - show it in the body language and the
    scene, not by naming the emotion.
  Aim for roughly 5-10 sentences per brief - enough to leave nothing
  ambiguous. Stay strictly grounded in this one continuous beat: do not
  narrate what happens before or after it, and do not use summary verbs
  like "then" or "later" to compress multiple moments into one brief.
...
exactly N objects, each shaped like this:
{
  ...
  "brief": "a dense, fully visual 5-10 sentence passage describing this exact beat - subjects, pose, setting, light, color, texture, atmosphere and in-frame motion - written as prose, never a plot summary",
...
def _parse_json_sequences(raw: str, n_expected: int) -> list[dict]:
    ...
    if len(data) != n_expected:
        raise ValueError(
            f"Le LLM a renvoyé {len(data)} séquence(s) au lieu des {n_expected} demandées."
        )
...
def sequence_to_brief(
    ...
    if video_music and video_music.strip():
        lines.append(f"\n{video_music.strip()}")
